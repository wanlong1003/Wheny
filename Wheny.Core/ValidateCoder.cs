...
namespace Wheny.Core
{
    ...
    public class ValidateCoder
    {
        ...
        public Bitmap CreateImage(string code, ValidateCodeType codeType)
        {
            ...
            for (int i = 0; i < code.Length; i++)
            {
                ...
                if (RandomItalic)
                {
                    ...
                    System.Drawing.Drawing2D.Matrix transform = grap.Transform;
        ...
        private static string GetRandomNums(int length)
        {
            ...
            var value = Random.Next((int)max);
            return value.ToString().PadLeft(length, '0');
        }

        private static string GetRandomNumsAndLetters(int length)
        {
            char[] allChars = new Char[] { '2', '3', '4', '5', '6', '7', '8', '9', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z' };
            List<char> result = new List<char>();
            while (result.Count < length)
            {
                int inx = Random.Next(allChars.Length);
                result.Add(allChars[inx]);
            }
